...
d_all <- d_raw %>%
  ...
  select(-c(
    rid, age, gender...568, hhi, ethnicity, education,
  ...
  mutate(
    ...
    educ_cat = factor(educ, 1:6, c(
      "< HS", "HS", "Some College",
      "2yr College", "4yr College", "Post-grad"
    )),
    income_cat = factor(income, 1:6, c(
      "<10K", "10-30K", "30-60K",
    ...
    dem_rep_num = case_when(
      party_id == "Democrat" ~ dem_strength, # 1 strong dem, # 2 not very strong dem
      party_id %in% c("Independent", "Other") & party_lean == 2 ~ 3, # leans dem
      party_id %in% c("Independent", "Other") & party_lean == 3 ~ 4, # no lean
      party_id %in% c("Independent", "Other") & party_lean == 1 ~ 5, # leans rep
      party_id %in% c("Republican") & rep_strength == 2 ~ 6, # not very strong rep,
      party_id %in% c("Republican") & rep_strength == 1 ~ 7, # strong rep
    ),
    ideology_cat = factor(
      ideo, 1:4,
    ...
    lib_con_num = case_when(
      ideology_cat == "Liberal" ~ lib_lean, # 1 strong lib, 2 not very strong
      ideology_cat %in% c("Moderate") & ideo_lean == 1 ~ 3, # lean lib
      ideology_cat %in% c("Moderate") & ideo_lean == 3 ~ 4, # no lean (moderate)
      ideology_cat %in% c("Moderate") & ideo_lean == 2 ~ 5, # lean conservative
      ideology_cat == "Conservative" & con_lean == 2 ~ 6, # not very strong con
      ideology_cat == "Conservative" & con_lean == 1 ~ 7, # strong con
    )
  ) |>
